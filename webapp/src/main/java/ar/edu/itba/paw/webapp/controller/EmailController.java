...
public class EmailController {
    ...
    @RequestMapping(method = RequestMethod.GET, path = "/{serviceId:\\d+}/post")
    public ModelAndView createAppointment(@PathVariable("serviceId") final long serviceId ) {
        ...
        String username = userEmail.substring(0, index)+"1";
        // ! }
        /*
        ...
        appointmentService.create(serviceId,user.getUserId(), startDate, startDate.plusMinutes(serv.getDuration()) );
*/
        try {
            emailService.sendMail(userEmail,"Waiting confirmation","",null);
